...
class DrapeVizApp:
    ...
    Y_TOP, Y_BOTTOM = 20.0, -120.0  # 시각화 상단이 20, 하단이 -120 (y축을 뒤집어 표시)
    ...
    def _build_frame_figure(self, frame_idx: int, show_grid: bool) -> go.Figure:
        """
    ...
    # ---------- Event Handlers ----------
    def on_frame_change(self, frame_idx: int, show_grid: bool) -> go.Figure:
        return self._build_frame_figure(frame_idx, show_grid)
    ...
    def build_interface(self) -> gr.Blocks:
        with gr.Blocks(title=self.title) as demo:
            ...
            # 프레임 변경 / 그리드 토글 시: 그림 갱신
            frame_slider.change(
                self.on_frame_change,
                inputs=[frame_slider, show_grid],
                outputs=[plot],
            )
            show_grid.change(
                self.on_frame_change,
